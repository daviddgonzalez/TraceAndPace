    ...
    void balanceTree(Node* node) {
        while (node) {
            ...
            if ((balanceFactor < -1 || balanceFactor > 1) && node->parent) {
                if (node->parent->values[0].first > node->values[0].first)
            ...
            // if there was a rotation then we need to check the height and subTreeSize
            if (balanceFactor < -1 || balanceFactor > 1) {
                if (node->childrenNodes[1]) {
                    ...
                    node->childrenNodes[1]->subTreeSize = (node->childrenNodes[1]->childrenNodes[1] ? node->childrenNodes[1]->childrenNodes[1]->subTreeSize : 0) + (node->childrenNodes[1]->childrenNodes[0] ? node->childrenNodes[1]->childrenNodes[0]->subTreeSize : 0) + 1;
                }
                if (node->childrenNodes[0]) {
                    ...
                    node->childrenNodes[0]->subTreeSize = (node->childrenNodes[0]->childrenNodes[0] ? node->childrenNodes[0]->childrenNodes[0]->subTreeSize : 0) + (node->childrenNodes[0]->childrenNodes[1] ? node->childrenNodes[0]->childrenNodes[1]->subTreeSize : 0) + 1;
                }
            }

            //should always update this nodes height and subTreeSize
            ...
            node->subTreeSize = (node->childrenNodes[0] ? node->childrenNodes[0]->subTreeSize : 0) + (node->childrenNodes[1] ? node->childrenNodes[1]->subTreeSize : 0) + 1;
            node = node->parent;
        }
    }
